import pytest
from src.utils.task import create_task, delete_task, get_a_task
...
def test_create_task_with_mandatory_fields(setup_create_delete_task, valid_token):
...
def test_create_task_with_optional_fields(setup_create_delete_task_optional_fields, valid_token):
    task_id = setup_create_delete_task_optional_fields
    response = get_a_task(task_id, valid_token)
    assert_create_task_successful(response)
...
def test_create_task_response_format(setup_create_delete_task, valid_token):
    task_id = setup_create_delete_task
    response = get_a_task(task_id, valid_token)
